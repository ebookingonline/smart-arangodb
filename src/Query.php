    ...
    */
    ...
    class Query extends Component implements QueryInterface
    {
        ...
        public function execute($aql, $bindValues = [], $params = [])
        {
            ...
            Yii::info($token, 'Smart\Arangodb\Query::query');
            try {
                Yii::beginProfile($token, 'Smart\Arangodb\Query::query');
                $cursor = $statement->execute();
                Yii::endProfile($token, 'Smart\Arangodb\Query::query');
            } catch (\Exception $ex) {
                Yii::endProfile($token, 'Smart\Arangodb\Query::query');
        ...
        public function insert($collection, $columns, $params = [], $db = null)
        {
            ...
            $token = $this->getRawAql($statement);
            Yii::info($token, 'Smart\Arangodb\Query::insert');
            try {
                Yii::beginProfile($token, 'Smart\Arangodb\Query::insert');
                $cursor = $statement->execute();
                Yii::endProfile($token, 'Smart\Arangodb\Query::insert');
            } catch (\Exception $ex) {
                Yii::endProfile($token, 'Smart\Arangodb\Query::insert');
                throw new \Exception($ex->getMessage(), (int)$ex->getCode(), $ex);
            }
        ...
        public function update($collection, $columns, $condition = [], $params = [], $db = null)
        {
            ...
            $statement = $this->getStatement($params, $db);
            $token = $this->getRawAql($statement);
            Yii::info($token, 'Smart\Arangodb\Query::update');
            try {
                Yii::beginProfile($token, 'Smart\Arangodb\Query::update');
                $cursor = $statement->execute();
                Yii::endProfile($token, 'Smart\Arangodb\Query::update');
            } catch (\Exception $ex) {
                Yii::endProfile($token, 'Smart\Arangodb\Query::update');
                throw new \Exception($ex->getMessage(), (int)$ex->getCode(), $ex);
            }
        ...
        public function remove($collection, $condition = [], $params = [], $db = null)
        {
            ...
            $statement = $this->getStatement($params, $db);
            $token = $this->getRawAql($statement);
            Yii::info($token, 'Smart\Arangodb\Query::remove');
            try {
                Yii::beginProfile($token, 'Smart\Arangodb\Query::remove');
                $cursor = $statement->execute();
                Yii::endProfile($token, 'Smart\Arangodb\Query::remove');
            } catch (\Exception $ex) {
                Yii::endProfile($token, 'Smart\Arangodb\Query::remove');
                throw new \Exception($ex->getMessage(), (int)$ex->getCode(), $ex);
            }
